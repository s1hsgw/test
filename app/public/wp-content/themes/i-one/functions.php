...
function ione_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Main Widget Area', 'i-one' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Appears in the footer section of the site.', 'i-one' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Main Sidebar Widget Area', 'i-one' ),
		'id'            => 'sidebar-2',
		'description'   => esc_html__( 'Appears on posts and pages in the sidebar.', 'i-one' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
...
function ione_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'i-one' ); ?></h1>
		<div class="nav-links">
...
function ione_entry_meta() {
	if ( is_sticky() && is_home() && ! is_paged() )
		echo '<span class="featured-post">' . esc_html( 'Sticky', 'i-one' ) . '</span>';

	if ( ! has_post_format( 'link' ) && 'post' == get_post_type() )
		ione_entry_date();

	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( esc_html__( ', ', 'i-one' ) );
	if ( $categories_list ) {
		echo '<span class="categories-links">' . $categories_list . '</span>';
	}

	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', esc_html__( ', ', 'i-one' ) );
	if ( $tag_list ) {
	...
	if ( 'post' == get_post_type() ) {
		printf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
			...
			esc_html( sprintf( __( 'View all posts by %s', 'i-one' ), get_the_author() ) ),
			get_the_author()
		);
	}
}
...
function ione_entry_date( $echo = true ) {
	...
	$date = sprintf( '<span class="date"><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>',
		...
		esc_html( sprintf( __( 'Permalink to %s', 'i-one' ), the_title_attribute( 'echo=0' ) ) ),
...
function ione_excerpt_more_link( $excerpt ){
    $post = get_post();
    $excerpt .= '<a class="moretag noptoppad" href="'. esc_url(get_permalink($post->ID)) . '">' . esc_html__( 'Read More...', 'i-one' ) . '</a>';
    return $excerpt;
}
...
*/
    $config = array(
        ...
        'strings' => array(
            'page_title' => esc_html__( 'Install Required Plugins', 'i-one' ),
            'menu_title' => esc_html__( 'Install Plugins', 'i-one' ),
            'installing' => esc_html__( 'Installing Plugin: %s', 'i-one' ), // %s = plugin name.
            ...
            'return' => esc_html__( 'Return to Required Plugins Installer', 'i-one' ),
            'plugin_activated' => esc_html__( 'Plugin activated successfully.', 'i-one' ),
            'complete' => esc_html__( 'All plugins installed and activated successfully. %s', 'i-one' ), // %s = dashboard link.
