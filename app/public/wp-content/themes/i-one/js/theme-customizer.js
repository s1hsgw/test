...
jQuery(document).ready(function($) {
	...
	/**/
	var otherShortcuts = $('<div class="tx-oth-shortcuts">\
	<div class="close-shortcut"><i class="far fa-times"></i></div>\
	<div class="tx-oths-items">\
	<span class="oths-item tx-sections">Activate/Deactivate/Sort Sections</span>\
	</div>\
	</div>');
	
	if( $('.txo-home').length > 0 )
	{
		$( otherShortcuts ).appendTo( '#page' );
		$( '#page' ).on( 'click', '.close-shortcut', function() {
			$('.tx-oth-shortcuts').css("visibility", "hidden");
		});		
	}
});
...
(function(wp,$,api){ 
	...
	api.bind( 'preview-ready', function() {
		...
		*/				
		...
		$( '#page' ).on( 'click', '.tx-sections', function() {
			wp.customize.preview.send( 'expand-sections-section' );			
		});			
